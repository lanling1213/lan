		...
		if (isset($_POST['userid'])) {
			...
			foreach($userarr as $v) {
				...
				if ($status > 0) {
					...
					$info['email'] = $v['email'];
					$info['regip'] = $v['regip'];
					$info['point'] = $v['modelid'] == 12 ? 0 : $v['point'];
					$info['groupid'] = $v['modelid'] == 12 ? 9 : $this->_get_usergroup_bypoint($v['point']);
			...
			if($_POST['sendemail']) {
				...
				$msg="您好！非常感谢您注册成为本网站的律师会员，请您在登录后去个人中心的个人资料里面完善个人资料，方便当事人咨询问题，若您通过本网站帮当事人（仅限本网站当事人会员）办理了案件 请您通过律师在线联系下管理员告知一声，我们会帮当事人会员增加积分  谢谢合作！";
				foreach ($memberinfo as $v) {
					sendmail($v['email'], L('reg_pass'), $_POST['message'] ? $_POST['message'] : $v['username']."（".$v['nickname']."）".$msg);
				}
			}
		...
		if(isset($_POST['userid'])) {
			...
			if($res) {
				if($_POST['sendemail']) {
					$memberinfo = $this->db->select($where);
					wy_base::load_sys_func('mail');
                    $msg = '律师，由于您的注册是填写的资料不够完整或者不真实，请您在注册的时候填写完整真实资料！谢谢合作';
					foreach ($memberinfo as $v) {
						sendmail($v['email'], L('reg_reject'), $_POST['message'] ? $_POST['message'] : $v['nickname'].$msg);
					}
				}
			}
