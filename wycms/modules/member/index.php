...
class index extends foreground
{
    ...
    public function register()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($member_setting['enablemailcheck']) {    //是否需要邮件验证
                $userinfo['groupid'] = 7;
            } elseif ($member_setting['registerverify'] && $userinfo['modelid'] == 12) {    //是否需要管理员审核(律师会员)
            ...
            if (wy_base::load_config('system', 'wysso')) {
                ...
                if ($status > 0) {
                    //查询推荐人
                    $referrer = $_POST['info']['referrer'];
                    $user = $this->db->get_one(array('username' => $referrer));
                    if ($user) {
                        //推荐人加50积分
                        $where = " userid = '$user[userid]' and groupid <> 9";
                        $this->db->update(array('point' => '+=50'), $where);
                    }
    ...
    public function login()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            //param::set_cookie('cookietime', $_cookietime, $cookietime);
            if ($r['modelid'] == 12) {
                $url = 'index.php?m=member&c=lawyer';
            } else {
                $url = 'index.php?m=member&c=index';
            }

            $forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : $url;
    ...
    protected function _get_usergroup_bypoint($point = 0)
    {
        ...
        if (isset($_GET['modelid']) && $_GET['modelid'] == 12) {
            return 9;
        }
    ...
    public function public_forget_password()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            header('Location:index.php?m=member&c=index&a=public_forget_password&type=3');
            exit;
        } elseif ($_GET['code']) {
            ...
            if (is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
                ...
                sendmail($email, L('forgetpassword'), "$memberinfo[nickname]，您好，你通过邮箱找回密码方式，系统帮您重置密码为：$password  ；若您要修改密码请登录后再会员中心修改，旧密码为重置密码。", '', '', $sitename);
     ...
     * 我推荐过的人
     */
    public function account_manage_referrer()
    {
        $memberinfo = $this->memberinfo;
        ...
        $grouplist = getcache('grouplist');
        $memberinfo['groupname'] = $grouplist[$memberinfo[groupid]]['name'];
        include template('member', 'account_manage_referrer');
    }

    /**
     * 咨询详情
     */
    public function account_ask_detail()
    {
        $t = intval($_GET['t']);
        ...
        $where = "`questionid`='$questionid' AND `memberid` = '$memberinfo[userid]'";
        ...
        $grouplist = getcache('grouplist');
        $memberinfo['groupname'] = $grouplist[$memberinfo[groupid]]['name'];
        include template('member', 'account_ask_detail');
    }

    /**
     * 推荐人详情
     */
    public function account_referrer_detail()
    {
        $referrerid = intval($_GET['rid']);
        if (empty($referrerid)) {
            showmessage("数据错误");
        }
        $memberinfo = $this->memberinfo;
        ...
        $referrer_detail = $member_model->get_one($where, '*');
        ...
        $grouplist = getcache('grouplist');
        $memberinfo['groupname'] = $grouplist[$memberinfo[groupid]]['name'];
        include template('member', 'account_referrer_detail');
    }

    /**
     * 专属律师
     */
    public function account_exclusive_lawyer()
    {
        $memberinfo = $this->memberinfo;
        ...
        $grouplist = getcache('grouplist');
        $memberinfo['groupname'] = $grouplist[$memberinfo[groupid]]['name'];
        include template('member', 'account_exclusive_lawyer');
    }

    /**
     * 会员购买商品列表
     */
    public function account_product_list()
    {
        $memberinfo = $this->memberinfo;
        ...
        $grouplist = getcache('grouplist');
        $memberinfo['groupname'] = $grouplist[$memberinfo[groupid]]['name'];
        include template('member', 'account_product_list');
    }
}
