...
class index extends foreground
{
    ...
    public function init()
    {
        ...
        if (empty($_userid)) {
            showmessage("请先进行登录再咨询问题", '/index.php?m=member&c=index&a=login');
        }
        if ($_groupid == 9) {
            showmessage("律师账号不支持提交咨询");
        }
        //模板风格使用站点全局设置风格
    ...
    public function ask()
    {
        ...
        if (isset($_POST['typeid']) && intval($_POST['typeid']) > 0) {
            ...
            $title = isset($_POST['title']) && trim($_POST['title']) != "" ? trim($_POST['title']) : str_cut($_POST['content'], 50, '');
    ...
    public function reply()
    {
        if (isset($_POST['qid']) && intval($_POST['qid']) > 0) {
            ...
            if ($this->reply_db->insert($reply_data) && $this->hits_db->update($hits_data, $where) && $this->db->update($ask_data, $where)) {
                if ($is_ajax) {
