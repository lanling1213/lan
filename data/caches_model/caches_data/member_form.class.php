...
class member_form {
	...
	function text($field, $value, $fieldinfo) {
		...
		return '<input type="text" name="info['.$field.']" id="'.$field.'" size="'.$size.'" value="'.$value.'" placeholder="'.$name.':" class="text" '.$formattribute.' '.$css.'>';
	}
	...
	function image($field, $value, $fieldinfo) {
		...
		if($show_type) {
			...
			<img src='$preview_img' id='{$field}_preview' width='135' height='113' style='cursor:hand' /></a><span style='float: left;'>添加律师头像（默认尺寸100*100)</span></div>";
		} else {
